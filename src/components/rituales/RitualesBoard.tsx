...
export default function RitualesBoard() {
	...
	const handleDeleteRitual = async (id: string, title: string) => {
		const toastKey = `delete-ritual-${id}`;
		sileo.error({
			id: toastKey,
			title: "¿Eliminar ritual?",
			fill: "#260f1c",
			duration: 3000,
			description: `Se removerá el ritual "${title}" de tus hábitos diarios y su tarea activa si existe. Pulsa 'Eliminar' para confirmar, o cierra esta alerta para cancelar.`,
			styles: {
			...
			button: {
				...
				onClick: () => {
					...
					removeRitualMutation({ id: id as Id<"rituals"> })
						.then(async () => {
						...
						.catch((err) => {
							...
							sileo.error({
								...
								description: "No se pudo eliminar el ritual. Intenta de nuevo.",
								fill: "#260f1c",
								styles: {
									title: "text-red-200 font-extrabold",
									description: "text-red-300/80 text-xs font-semibold mt-0.5",
